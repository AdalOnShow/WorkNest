...
const MOCK_TASKS = [
  { id: '1', title: 'Fix login bug', assignee: 'John D.', status: 'IN_PROGRESS' as const, priority: 'HIGH' as const, due: 'Feb 20' },
  { id: '2', title: 'Add tests', assignee: 'Jane S.', status: 'TODO' as const, priority: 'MEDIUM' as const, due: 'Feb 25' },
  { id: '3', title: 'Design UI', assignee: 'Mike R.', status: 'COMPLETED' as const, priority: 'LOW' as const, due: '—' },
]
...
const MOCK_MESSAGES = [
  { id: '1', author: 'John D.', text: 'Hey team, ready to start?', time: '10:30 AM', isOwn: false },
  { id: '2', author: 'Jane S.', text: "Yes! Let's go.", time: '10:32 AM', isOwn: false },
  { id: '3', author: 'You', text: "I'll handle the frontend.", time: '10:35 AM', isOwn: true },
]
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="body-md text-muted-foreground mt-1 max-w-2xl">{MOCK_PROJECT.description}</p>
            <div className="flex items-center gap-4 mt-3">
              <StatusBadge status={MOCK_PROJECT.status} />
              <span className="body-sm text-muted-foreground">Deadline: {MOCK_PROJECT.deadline}</span>
            </div>
          </div>
        ...
        <Tabs defaultValue="tasks" className="w-full">
          <TabsList className="bg-card border border-border p-1 rounded-lg w-fit">
            <TabsTrigger value="tasks" className="data-[state=active]:bg-accent data-[state=active]:text-primary rounded-md px-6">
              Tasks
            </TabsTrigger>
            <TabsTrigger value="members" className="data-[state=active]:bg-accent data-[state=active]:text-primary rounded-md px-6">
              Members
            </TabsTrigger>
            <TabsTrigger value="chat" className="data-[state=active]:bg-accent data-[state=active]:text-primary rounded-md px-6">
              Chat
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="tasks" className="mt-6 space-y-4">
            ...
            <div className="flex justify-center">
              <Pagination currentPage={taskPage} totalPages={2} onPageChange={setTaskPage} />
            </div>
          </TabsContent>
              ...
              <Table>
                ...
                <TableBody>
                  {MOCK_MEMBERS.map((member) => (
                    <TableRow key={member.id} className="border-border/50 hover:bg-accent/30">
                      <TableCell className="text-foreground font-medium">{member.name}</TableCell>
                      <TableCell className="text-card-foreground">{member.email}</TableCell>
                      <TableCell>
                        <span className={`label-sm ${
                          member.role === 'ADMIN' ? 'text-primary' :
                          member.role === 'PM' ? 'text-[var(--color-status-in-progress)]' : 'text-card-foreground'
                        }`}>
                          {member.role}
                        </span>
                      </TableCell>
          ...
          <TabsContent value="chat" className="mt-6">
            <div className="bg-card border border-border rounded-[14px] overflow-hidden">
              <div className="p-4 border-b border-border">
                <h3 className="label-lg text-foreground">Chat — {MOCK_PROJECT.name}</h3>
              </div>
